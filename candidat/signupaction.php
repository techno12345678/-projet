...
if(!empty($_POST['name'])  AND !empty($_POST['email'] ) AND !empty($_POST['password']  )){
    ...
    $checkIfUserAlreadyExists =$db->prepare('SELECT email FROM candidatlogin WHERE email = ?');
    $checkIfUserAlreadyExists->execute(array($user_email));

    if($checkIfUserAlreadyExists->rowCount() == 0){
            
        //Insérer l'utilisateur dans la bdd
        $insertUserOnWebsite =$db->prepare('INSERT INTO candidatlogin(name, email, password)VALUES(?, ?, ?)');
        $insertUserOnWebsite->execute(array($user_name, $user_email,  $user_password));
 //Récupérer les informations de l'utilisateur
 $getInfosOfThisUserReq =$db->prepare('SELECT id,  name,email FROM candidatlogin WHERE name = ? AND email = ?');
